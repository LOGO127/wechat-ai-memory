...
- WXGF 高清图片解码与语音处理共用 PyAV，移除重复的独立 FFmpeg EXE，保留图片与语音功能。
- 排除未使用的 Qt 虚拟键盘、PDF 图片插件及其依赖，保留原生输入、SVG 图标和图形后备库。
- 教学动画制作依赖移入独立的 `media` 可选依赖组，不进入用户软件包。
- 隔离打包时的环境变量，避免从开发工具 PATH 混入额外的原生运行库。

### Tests

- 新增无损 WXGF 逐像素回归测试与 Qt 插件筛选测试。
- 两种打包 EXE 增加合成图片解码、SILK 音频重采样、CPU 推理库和 VAD 实际推理验收，不依赖开发机 FFmpeg 或联网下载模型。
- 发布检查记录便携版、ZIP、解压目录的实际字节数，并阻止重复依赖和超出预算的软件包通过验收。
- 本地 Windows x64 构建：便携版从 210.3 MiB 降至 169.4 MiB，ZIP 从 216.2 MiB 降至 173.9 MiB；45 项源码测试及两种 EXE、ZIP 解压版的运行检查通过。
