...
class ChatRenderer:
    def __init__(
        ...
        self.title_font = self.fonts.bold(34)
        self.header_meta_font = self.fonts.regular(18)
        self.sender_font = self.fonts.bold(20)
        self.body_font = self.fonts.regular(25)
        self.small_font = self.fonts.regular(17)
        self.date_font = self.fonts.bold(17)
        self.empty_font = self.fonts.regular(26)
    ...
    def _build_blocks(self, messages: list[Message]) -> list[_DateBlock | _MessageBlock]:
        ...
        for message in messages:
            ...
            if message.type is MessageType.IMAGE:
                ...
                width = thumbnail.width if thumbnail else 580
                height = 30 + (thumbnail.height if thumbnail else 220) + 44
                blocks.append(_MessageBlock(message, [], max(width + 30, 250), height, thumbnail=thumbnail))
                continue

            content = self._display_content(message)
            lines = _wrap_text(draw, content, self.body_font, max_text_width)
            chunks = [lines[index : index + max_lines] for index in range(0, len(lines), max_lines)] or [[""]]
            for chunk_index, chunk in enumerate(chunks):
                line_width = max((int(draw.textlength(line, font=self.body_font)) for line in chunk), default=0)
    ...
    def _draw_header(
        ...
        draw.text((self.config.margin_x, 24), conversation.name, font=self.title_font, fill=self.theme.ink)
    ...
    def _draw_message(self, draw: ImageDraw.ImageDraw, page: Image.Image, block: _MessageBlock, y: int) -> None:
        ...
        draw.text((left, y + 3), sender, font=self.sender_font, fill=self.theme.ink)
        ...
        if message.type is MessageType.IMAGE:
            if block.thumbnail:
                image_x = left + (block.width - block.thumbnail.width) // 2
                image_y = bubble_top + 15
                page.paste(block.thumbnail, (image_x, image_y))
                draw.rounded_rectangle(
                    (image_x, image_y, image_x + block.thumbnail.width, image_y + block.thumbnail.height),
                    radius=6,
        ...
        if message.reply_to and not block.continuation:
            ...
            draw.text((left + 27, text_y + 4), f"Reply to {message.reply_to}", font=self.small_font, fill=self.theme.muted)
            text_y += 38
        for line in block.lines:
            draw.text((left + 18, text_y), line, font=self.body_font, fill=self.theme.ink)
            text_y += 38
    ...
    def _draw_system_message(self, draw: ImageDraw.ImageDraw, block: _MessageBlock, y: int) -> None:
        width = min(
            ...
            max((draw.textlength(line, font=self.small_font) for line in block.lines), default=0) + 36,
        )
        x = (self.config.width - width) / 2
        draw.rounded_rectangle((x, y + 10, x + width, y + block.height - 12), radius=8, fill="#E8ECEF")
        text_y = y + 23
        for line in block.lines:
            line_width = draw.textlength(line, font=self.small_font)
            draw.text(((self.config.width - line_width) / 2, text_y), line, font=self.small_font, fill=self.theme.muted)
            text_y += 30
    ...
    for paragraph in text.replace("\r\n", "\n").replace("\r", "\n").split("\n"):
        ...
        for token in tokens:
            ...
            if current and draw.textlength(candidate, font=font) > max_width:
                ...
                current = candidate
            while draw.textlength(current, font=font) > max_width:
